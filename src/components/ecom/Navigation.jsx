    ...
    renderDepartments () {
        ...
        return this.props.departments.map(({ name, department_id }) => {
            return (
                    ...
                    >
                    <div className={ `text ${ Object.values(this.props.categories).length > 0 ? ( this.props.categories.some(item => item.department_id === department_id ) ? (this.state.active ? 'active' : '' ) :'') : ''}` } role="alert" aria-live="polite"  id={ department_id } 
                       
                       onClick={ this.departmentClick } > { name }</div> 
