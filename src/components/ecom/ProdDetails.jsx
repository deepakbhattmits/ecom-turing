...
class ProdDetails extends Component {
    ...
    componentDidMount(){
        const prod_id  = this.props && this.props.match.params.id ? this.props.match.params.id : '' ;  
        ...
        return this.props.prodsAttr.color.map(attr => {
                       return (
                         <button 
                                ...
                                className={ `ui ${attr.attribute_value.toLowerCase()} circular label ${attr.attribute_name}` } 
                            >
            ...
            cartData: [{
                        ...this.state.cartData, 
                           // eslint-disable-next-line no-useless-computed-key
                            ['item_id'] : this.props.uniqueCartId.cart_id ? this.props.uniqueCartId.cart_id : '',
                            // eslint-disable-next-line no-useless-computed-key
                            ['name'] : (this.props && this.props.prodData )? this.props.prodData.name : '',
                            // eslint-disable-next-line no-useless-computed-key
                            ['product_id'] : this.props.prodData ? this.props.prodData.product_id : '',
                            // eslint-disable-next-line no-useless-computed-key
                            ['attributes'] : `${this.state.color? this.state.color: ' '},${this.state.size? this.state.size:' '}`,
                            // eslint-disable-next-line no-useless-computed-key
                            ['price'] : this.props.prodData ? this.props.prodData.discounted_price : '',
                            // eslint-disable-next-line no-useless-computed-key
        ...
        // rtData).length > 1) {
            ...
            for (var key in this.state.cartData) {
                if(key === 'attributes') {
                    ...
                    if(  this.state.cartData[key].includes(' , ') || this.state.cartData[key].includes(' ,') || this.state.cartData[key].includes(', ')){
                        this.setState({ validate: true },() =>{ this.throwErrormsg()}) 
                    }
                }
    ...
    render () {    
        ...
        const { product_id, name, description, price, discounted_price, image, image_2, thumbnail  } = (this.props && this.props.prodData )? this.props.prodData : '';
        var items = [];
        const graphImage = image ? require(`../../assets/product_images/${ image }`) : '' ; 
        const Aimage_2 = image_2 ? require(`../../assets/product_images/${ image_2 }`) : '' ; 
        const Athumbnail = thumbnail ? require(`../../assets/product_images/${ thumbnail }`) : '' ;
        ...
        return (
            <div className={`ui grid left aligned`}> 
                ...
                <div className="item">
                    { round ?
                    <Fragment>
                        <button className="button basic ui" onClick={ (e) => { this.setState({status: !this.state.status })} }>
                    ...
                    <div className="item">
                        <span className="ui red lebel big ">{ `Regular Price :${price ? price : '' } Discounted Price :${discounted_price ? discounted_price : '' }`}</span>
                    </div>
                    <div className="item">
                    ...
                    <div className="item">
                        <div className="ui search focus">
                            <button className="ui circular icon button" onClick={ (e) => { this.setState({ quantity :  this.state.quantity + 1} )} }>
                                <i className="icon plus"></i>
                            </button>
                            <input className="prompt"  style={{ textAlign: 'center' }} onChange={ (e) => { this.setState({ quantity: this.state.quantity } ) } } value={ this.state.quantity }/>
                            <button className="ui circular icon button" onClick={ (e) => { this.setState({ quantity: this.state.quantity >= 2 ?  this.state.quantity -1  :  this.state.quantity}) } }>
                    ...
                    <div className={`ui error message ${this.state.validate ? '' : 'transition hidden' }`}>
